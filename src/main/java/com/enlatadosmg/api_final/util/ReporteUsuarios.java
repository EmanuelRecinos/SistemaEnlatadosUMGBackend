...
public class ReporteUsuarios {
    ...
    public static void generarReporte(Lista listaUsuarios) throws IOException {
        ...
        // Guardamos el archivo .dot en el sistema de archivos
        File file = new File("reporte_usuarios.dot");
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(file))) {
            writer.write(dotContent.toString());
        }

        System.out.println("Reporte generado: reporte_usuarios.dot");
    }
    ...
    public static void generarImagen() throws IOException {
        ...
        File dotFile = new File("reporte_usuarios.dot");
        if (dotFile.exists()) {
            String command = "dot -Tpng reporte_usuarios.dot -o reporte_usuarios.png";
            Process process = Runtime.getRuntime().exec(command);
            try {
                process.waitFor();
                System.out.println("Imagen generada: reporte_usuarios.png");
